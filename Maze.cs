...
namespace MajorProjectDesktop
{
    class Maze
    {
        ...
        public Maze(int h, int w)
        {
            ...
            DisplayCellsCount = 0;
            _renderer = new ConsoleRenderer(w, h, Cellsize);
            for (int y = 0; y < h; y++)
            {
                for (int x = 0; x < w; x++)
                {
                ...
                for (int n = 0; n < Cellsize - 1; n++) //Prints every cell in this row three times
                {
                    for (int Hcells = 0; Hcells < w; Hcells++) //Goes through every cell in a row
                    {
                        ...
                        if (CellList[Hcells, Vcells].Walls[0] == true)
                        {
                            ...
                            {
                                if (CellList[Hcells, Vcells].Path)
                                {
                                    _renderer.AddColoredCellToBuffer(ConsoleRenderer.GreenBG, Cellsize);
                                }
                                else
                                {
                                    _renderer.AddColoredCellToBuffer(ConsoleRenderer.WhiteBG, Cellsize);
                                }
                            }
